...
### Added
- **Approved side-chat capabilities** (PRD-0007, ADR-0007, issue #57): `/gremlins:chat` and `/gremlins:tangent` remain conversational-only with explicit `tools: []` by default, but project `.pi/settings.json` can now opt each mode into approved read-only tools (`read`, `grep`, `find`, `ls`) and project-local `.pi/skills/` Markdown skill files with fail-closed validation for unsafe tools, unsafe paths, missing `read`, and skill-loader diagnostics.
