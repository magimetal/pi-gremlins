...
The architecture changed again for issue #57. Side-chat should not introduce or depend on a pi-gremlins-specific side-chat capability configuration file shape. Instead, a side-chat child session should use the Pi SDK's default built-in tools by omitting the `tools` option when creating the child session. Extension-provided custom tools should be made available through a fresh side-chat resource-loading path that reloads enabled extensions without inheriting parent prompt, transcript, `AGENTS`, prompt, theme, or parent-loaded skill context. Follow-on issue #59 narrows the skill rule: parent-loaded and legacy side-chat skill context remain stripped, but fresh child-session skills and diagnostics may load through the child `DefaultResourceLoader` so SDK-native skill guidance can reach the side-chat runtime.
...
- Preserve side-chat isolation from parent transcript/history, system prompt, primary-agent markdown, prompts, themes, parent-loaded skills, `AGENTS` files, and unapproved parent prompt material.
...
- Allow intended fresh child-session skills and diagnostics through that same child resource-loader boundary for SDK-native skill guidance.
...
- Cons:
  ...
  - Risks leaking parent transcript, prompt, `AGENTS`, parent-loaded skills, prompts, themes, and extension context into the side-chat.
...
- Pros:
  ...
  - Keeps custom tool and child skill discovery aligned with enabled resources while using a fresh side-chat resource loader rather than parent runtime state.
  - Preserves the ability to keep parent transcript, prompt, and parent-loaded skill surfaces isolated from side-chat.
- Cons:
...
Rationale: Side-chat should rely on the same default built-in tool behavior as a normal SDK child session instead of maintaining a parallel pi-gremlins allowlist or side-chat `.pi/settings.json` contract. Custom extension tools and fresh child-session skills are still useful, but they must be exposed through a fresh side-chat `DefaultResourceLoader` path so side-chat can discover enabled extension tools and SDK-native skill guidance without inheriting the parent session's prompt, history, `AGENTS`, prompts, themes, parent-loaded skills, or other context surfaces.
...
- Use a fresh side-chat child `DefaultResourceLoader` to reload enabled extensions, expose custom extension tools, and load fresh child-session skills/diagnostics.
- Strip, ignore, or avoid parent and non-skill non-tool surfaces where possible so prompts, themes, parent-loaded skills, legacy side-chat skill paths, `AGENTS`, primary-agent markdown, and parent prompt material do not leak into side-chat context.
...
- Keep side-chat system prompt text explicit about the side-chat boundary, SDK-default built-in tools, extension custom tool availability, fresh child-session skill guidance, and prohibited parent-context/resource inheritance.
...
- Future changes that add side-chat-specific config, alter default tool policy, inherit parent-loaded skills, or allow non-skill non-tool extension surfaces into side-chat require a new ADR or supersession.
...
- **Positive:** Extension custom tools and fresh child-session skills can be available to side-chat without reusing parent session state.
...
- **Migration/ops:**
  ...
  - Side-chat resource loading uses a fresh child `DefaultResourceLoader` path for enabled extension custom tools and fresh child-session skills/diagnostics.
  - Documentation should state that side-chat uses SDK-default built-in tools, extension custom tools, and fresh child-session skill guidance, while parent prompt/history/resource inheritance remains prohibited.
...
- **Automated:**
  ...
  - Resource-loader tests prove a fresh side-chat child `DefaultResourceLoader` exposes enabled extension custom tools and fresh child-session skills/diagnostics.
  - Negative tests prove parent transcript/history, prompt, primary-agent markdown, prompts, themes, parent-loaded skills, legacy side-chat skill paths, `AGENTS`, and non-skill non-tool extension surfaces do not leak into side-chat context where the implementation can filter them.
  - Prompt tests prove `chat` and `tangent` prompts accurately describe SDK-default built-in tools, extension custom tools, fresh child-session skill guidance, and parent-context isolation.
...
- **Manual:**
  ...
  - Verify extension prompts/themes/parent-loaded skills/legacy side-chat skill paths/`AGENTS`/primary-agent material are absent from side-chat context unless a future ADR explicitly allows them.
...
This decision authorizes tools and fresh child-session skill guidance, not parent state sharing. Side-chat capability does not authorize inheriting the parent session transcript, prompt, primary-agent markdown, prompts, themes, parent-loaded skills, `AGENTS`, or non-skill non-tool extension resources.

## Status History

- 2026-05-01: Revised for issue #59; fresh child-session skills and diagnostics are allowed through the side-chat child resource loader while parent-loaded skills and legacy side-chat skill paths remain prohibited.
