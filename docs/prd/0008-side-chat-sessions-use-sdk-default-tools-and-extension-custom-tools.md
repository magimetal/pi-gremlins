...
- As an operator, I can trust that side-chat does not inherit the parent transcript, live history, AGENTS files, prompts, themes, or primary-agent material except where needed for extension tool operation.
...
- Preserve isolation from parent transcript/history, parent prompts, themes, AGENTS files, primary-agent markdown, and unrelated parent resources unless a specific extension tool requires its own extension-provided runtime context.
...
- [x] No side-chat-specific `.pi/settings.json` configuration, `pi-gremlins.sideChat` allowlist, per-mode capability profile, or side-chat skill path list is required or read for issue #57 behavior.
...
- [x] Side-chat does not inherit parent live history, AGENTS files, prompts, themes, primary-agent markdown, or unrelated parent resources except as required by enabled extension tool operation.
...
- **Side-chat session factory:** `extensions/pi-gremlins/side-chat-session-factory.ts` — should rely on SDK default built-in tools plus extension custom tools rather than explicit `tools: []` or side-chat allowlists.
