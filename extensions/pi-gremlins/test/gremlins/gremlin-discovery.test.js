...
describe("gremlin discovery v1 contract", () => {
	test("loads only user and nearest project gremlins and lets project override same-name user gremlin", async () => {
		const { createGremlinDiscoveryCache } = await import("../../gremlins/gremlin-discovery.ts");
	...
	test("bounds fingerprint stat fan-out for large markdown file sets", async () => {
		const { DEFAULT_DISCOVERY_FILE_CONCURRENCY, createGremlinDiscoveryCache } = await import("../../gremlins/gremlin-discovery.ts");
		const workspace = createWorkspace();
		workspaceRoot = workspace.root;
		fs.mkdirSync(workspace.projectAgentsDir, { recursive: true });
