...
describe("gremlin session factory v1 contract", () => {
	test("builds child session config from raw gremlin markdown caller intent and caller context only", async () => {
		const { buildGremlinPrompt } = await import("../../gremlins/gremlin-prompt.ts");
		const { buildGremlinSessionConfig } = await import(
			"../../gremlins/gremlin-session-factory.ts"
		);
	...
	test("reports ambiguous bare gremlin model ids with provider-qualified matches", async () => {
		const { resolveGremlinModel } = await import("../../gremlins/gremlin-session-factory.ts");
		const modelRegistry = {
			getAll: () => [
				{ provider: "openai", id: "gpt-5" },
